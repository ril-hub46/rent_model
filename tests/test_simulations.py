...
def test_sweep_price_runs_model_for_each_price(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def run_model_side_effect(inputs, regime, gold_price, discount_rate=None,
                              royalty_rate_override=None, cit_rate_override=None):
    ...
    df = sweep_price_from_excel(excel_path="dummy.xlsx", prices=prices, regime_code="CM2003", mine_sheet="MineSheet", amort_sheet="AmortSheet", discount_override=0.12, royalty_rate_override=0.08, cit_rate_override=0.27)
    ...
    expected_cols = ["regime", "gold_price", "discount_rate", "royalty_rate", "cit_rate", "NPV_pre_tax", "NPV_post_tax","Gov_NPV","TEMI"]
    assert list(df.columns) == expected_cols
    assert df["gold_price"].tolist() == prices
    assert set(df["discount_rate"].tolist()) == {0.12}


def test_sweep_price_without_overrides_passes_none(monkeypatch: pytest.MonkeyPatch) -> None:
    prices = [1000.0, 1100.0]
    fake_regime = object()
    fake_inputs = object()
    monkeypatch.setattr(sim, "default_regime", Mock(return_value=fake_regime))
    monkeypatch.setattr(sim, "load_project_inputs_from_excel", Mock(return_value=fake_inputs))
    ...
    df = sweep_price_from_excel(excel_path="dummy.xlsx", prices=prices, regime_code="CM2015", mine_sheet=None, amort_sheet="Amortissement")
