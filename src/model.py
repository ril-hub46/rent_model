...
def _read_scalar_from_sheet(
    df: pd.DataFrame, label_regex: str, fallback: float = np.nan
) -> float:
    """
...
def load_project_inputs_from_excel(
    ...
    if mine_sheet is None:
        mine_sheet = (
            "Données de la mine" if rc in {"CM2003", "2003"} else "Code Minier 2015"
        )
    ...
    if revenue is None:
        raise ValueError(
            f"Revenue row not found on '{mine_sheet}' (expected 'Chiffre d\\'Affaire')."
        )
    ...
    base_gold_price = _read_scalar_from_sheet(
        mine_df, r"Cours\s+de\s+l['’]or", fallback=np.nan
    )
    discount_rate = _read_scalar_from_sheet(
        mine_df, r"Taux\s+d['’]actualisation", fallback=np.nan
    )
    ...
    if dep is None:
        ...
        dep = (dep1.fillna(0) if dep1 is not None else 0) + (
            dep2.fillna(0) if dep2 is not None else 0
        )

    common_years = np.intersect1d(years, amort_years)
    if common_years.size == 0:
        raise ValueError(
            "No overlapping years between mine sheet and amortization sheet."
        )
    ...
    if not np.isfinite(base_gold_price):
        ...
        if price_row is not None:
            base_gold_price = float(
                pd.to_numeric(price_row.dropna().iloc[0], errors="coerce")
            )
        else:
...
def run_model(
    ...
    price = (
        float(gold_price) if gold_price is not None else float(inputs.base_gold_price)
    )
    ...
    dr = (
        ...
        else float(inputs.discount_rate)
    )

    rr = (
        float(royalty_rate_override)
        if royalty_rate_override is not None
        else royalty_rate_for_price(regime, price)
    )
    cit_rate = (
        float(cit_rate_override)
        if cit_rate_override is not None
        else float(regime.cit_rate)
    )
